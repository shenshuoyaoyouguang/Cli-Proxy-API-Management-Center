...
const getResolvedModelTokens = (modelData: Record<string, unknown>, details: unknown[]): number => {
  ...
  const hasTokenEvidence = details.some((detail) => hasUsageTokenEvidence(detail));
  return hasTokenEvidence || explicitTokens === 0 ? derivedTokens : explicitTokens;
};
...
const getResolvedAggregateValue = (
  ...
  preferDerived ? derivedValue : toFiniteNonNegativeNumber(explicitValue);
